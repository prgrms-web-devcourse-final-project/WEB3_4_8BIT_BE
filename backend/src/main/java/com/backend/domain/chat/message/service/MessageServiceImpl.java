...
public class MessageServiceImpl implements MessageService {
	...
	@Override
	public MessageResponse saveMessage(final MessageRequest request) {
		...
		// 2. Message 도큐먼트 생성
		Message message = MessageConverter.toEntity(request, fileUrls);

		// 3. 저장
		Message saved = messageRepository.save(message);

		// 4. Sender 프로필 이미지 URL 조회
		String profileImageUrl = memberService.getMemberFileUrl(request.senderId());

		// 5. 응답 객체로 변환 후 반환
		return MessageConverter.toResponse(saved, profileImageUrl);
	}
}
